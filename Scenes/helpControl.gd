...
func _on_button_pressed() -> void:
	# If the popup is already active, remove it (toggle off)
	if active_pop_up != null:
		active_pop_up.queue_free()  # Removes the popup from the scene
		active_pop_up = null
	else:
		# Create an instance of the popup scene (toggle on)
